...
INT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		case WM_CREATE:
		{
			...
			SetSkin(hwnd, g_SKIN[indexTheme]);
			SetFont(hEdit, g_FONT[indexFont]);
		}
		break;
		...
		case WM_CTLCOLOREDIT:
			{
				...
				SetBkColor(hdcEdit, g_DISPLAY_BACKGROUND_COLOR[indexTheme]);
				SetTextColor(hdcEdit, g_DISPLAY_FOREGROUND_COLOR[indexTheme]);

				HBRUSH hbrBackground = CreateSolidBrush(g_WINDOW_BACKGROUND_COLOR[indexTheme]);
		...
		case WM_CONTEXTMENU:
			{
				...
				InsertMenu(hMenu, -1, MF_BYPOSITION | MF_STRING | MF_DISABLED, 0, "Tema");
				InsertMenu(hMenu, -1, MF_BYPOSITION | MF_SEPARATOR, 0, NULL);

				InsertMenu(hMenu, -1, MF_BYPOSITION | MF_STRING | MF_UNCHECKED, IDR_SQUARE_BLUE, "Square blue");
				InsertMenu(hMenu, -1, MF_BYPOSITION | MF_STRING | MF_UNCHECKED, IDR_METAL_MISTRAL, "Metal mistral");
				InsertMenu(hMenu, -1, MF_BYPOSITION | MF_STRING | MF_DISABLED, 0, "");
				CheckMenuItem(hMenu, (2 + indexTheme), MF_BYPOSITION | MF_CHECKED);

				InsertMenu(hMenu, -1, MF_BYPOSITION | MF_STRING | MF_DISABLED, 0, "Шрифт");
				InsertMenu(hMenu, -1, MF_BYPOSITION | MF_SEPARATOR, 0, NULL);

				InsertMenu(hMenu, -1, MF_BYPOSITION | MF_STRING | MF_UNCHECKED, IDR_AZONIX, "Azonix");
				InsertMenu(hMenu, -1, MF_BYPOSITION | MF_STRING | MF_UNCHECKED, IDR_MOSCOW2024, "MOSCOW 2024");
				InsertMenu(hMenu, -1, MF_BYPOSITION | MF_STRING | MF_DISABLED, 0, "");
				CheckMenuItem(hMenu, (7 + indexFont), MF_BYPOSITION | MF_CHECKED);

				InsertMenu(hMenu, -1, MF_BYPOSITION | MF_SEPARATOR, 0, NULL);
				InsertMenu(hMenu, -1, MF_BYPOSITION | MF_STRING, IDR_EXIT, "Exit");
				...
				switch (item)
					{
						...
						case IDR_METAL_MISTRAL:  
							indexTheme = item - IDR_SQUARE_BLUE;
							break;

						case IDR_AZONIX:
						case IDR_MOSCOW2024:
							indexFont = item - IDR_AZONIX;
							break;
				...
				SetSkin(hwnd, g_SKIN[indexTheme]);
				SetFont(hEditDisplay, g_FONT[indexFont]);
...
{
	CHAR sz_filename[MAX_PATH]{};
	sprintf(sz_filename, "Fonts\\%s.otf", font);
